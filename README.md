...
Enter the API base, normally ending in `/v1`, in **AI / Ollama Configuration**. A bearer token can be entered for the current browser session. NotebookLM+ deliberately does **not** persist that token in IndexedDB or exported notebook backups.
...
If the richer picker is not available, the app falls back to ordinary browser file/directory selection. The selected bytes are parsed locally and the resulting index is stored, but the browser may require the source to be selected again for a future rescan.
...
- 7 retrieval/configuration unit tests;
- 4 Ollama API integration tests against a mock HTTP Ollama server;
- 10 static application-contract tests;
