...
- Eight configurable modes: **CPU / No GPU, Lightweight (Local Ollama), Local Ollama, Balanced (Local Ollama), Power (Local Ollama), Remote Ollama, Hosted AI Engine, Custom**.
...
- Collapsible source tree with nested folders, individual file checkboxes, parent-folder selection, and source scopes: entire notebook or selected files/folders.
...
7. configure API/embedding timeout and keep-alive;
8. configure context size, top-K retrieval, batch size, workers and temperature;
9. configure separate model-load/first-response and streaming-inactivity timeouts;
10. save the Ollama and mode settings.
...
|---|---:|---:|---|
| CPU / No GPU | 4,096 | 4 | CPU-only machines; keyword-first retrieval, one worker, long first-response timeout |
...
Use this mode on machines without usable graphics acceleration. Its defaults are deliberately conservative:

- 4,096-token context;
- top-K 4;
- maximum answer length 384 tokens;
- one indexing worker;
- semantic embeddings off by default (keyword retrieval still works);
- Ollama keep-alive 30 minutes to avoid repeatedly loading the model;
- model-load / first-response timeout 900 seconds;
- streaming inactivity timeout 240 seconds.

The two chat timeouts are intentionally separate. A CPU-only model may take a long time to load and produce its first token. Once the response starts streaming, NotebookLM+ resets the inactivity timer whenever new data arrives, so a slow but healthy answer is not killed merely because its total duration is long.

For CPU-only use, a smaller quantized chat model is usually the single biggest speed improvement. If embeddings are not essential, leave the Embedding model on **Keyword-only** so Ollama does not have to load a second model during indexing/retrieval.
...
### Collapsible source tree

The right-hand Sources panel preserves the source hierarchy:

- a multi-file selection expands to individual files;
- a selected folder expands to its sub-folders and files;
- source and folder nodes are collapsible;
- each source, folder, sub-folder and file has a checkbox;
- partially selected folders show an indeterminate checkbox;
- changing a checkbox automatically switches chat scope to **Selected files/folders**;
- selected retrieval filters by document ID, so excluding one file does not remove its sibling files from the notebook.
...
- 20 retrieval/configuration/parser/security/fuzz unit tests;
- 8 Ollama API protocol/integration tests against a mock HTTP Ollama server;
- 9 hosted OpenAI-compatible protocol/integration tests;
- 30 static application-contract tests;
...
- all eight CPU/local/remote/hosted/custom modes;
...
## v0.6.0 source-tree + CPU hardening

- Added collapsible multi-file/folder source tree with nested sub-folders.
- Added per-file and per-folder include/exclude checkboxes.
- Selected retrieval now filters at document level, not only source level.
- Added dedicated **CPU / No GPU** performance mode.
- Split chat timing into model-load/first-response timeout and streaming-inactivity timeout.
- Increased configurable API/embedding timeout range to 1,800 seconds.
- CPU mode disables semantic embeddings by default, uses one index worker and keeps Ollama loaded for 30 minutes.
- Source-tree rendering uses document chunk counts instead of loading every chunk merely to display the panel.
- Added source-tree unit/contract tests and a delayed-first-response Ollama integration test.
