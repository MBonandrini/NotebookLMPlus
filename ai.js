...
async function fetchHosted(endpoint, path, init={}, timeoutSeconds=120, apiKey=getSessionApiKey()) {
  ...
  const armTimeout = () => {
    ...
    timeout = setTimeout(() => controller.abort(new DOMException('Request timed out', 'TimeoutError')), timeoutSeconds * 1000);
  };
  armTimeout();
  try {
...
export async function streamAiChat({ provider='ollama', endpoint, apiKey=getSessionApiKey(), model, messages, contextTokens=16384, maxAnswerTokens=1280, temperature=0.18, timeoutSeconds=120, keepAlive='5m', signal=null, onToken=()=>{}, onStats=()=>{} }) {
  provider = normalizeProvider(provider);
  if (provider === 'ollama') {
    return streamOllamaChat({
      endpoint, model, messages, timeoutSeconds, keepAlive, signal,
  ...
  try {
    ...
    while (!done) {
      ...
      if (streamDone) break;
      armTimeout(); // inactivity timeout: a healthy slow generation may run longer than timeoutSeconds total
